...
class FastRCNNOutputs(object):
    ...
    def __init__(
        self, box2box_transform, pred_class_logits, pred_proposal_deltas, proposals, smooth_l1_beta, num_classes, type="instance"
    ):
        """
        Args:
    ...
    def _log_accuracy(self):
        ...
        storage.put_scalar(self.type+"_fast_rcnn/cls_accuracy", num_accurate / num_instances)
        if num_fg > 0:
            storage.put_scalar(self.type+"_fast_rcnn/fg_cls_accuracy", fg_num_accurate / num_fg)
            storage.put_scalar(self.type+"_fast_rcnn/false_negative", num_false_negative / num_fg)
    ...
    def losses(self):
        ...
        return {
            "loss_"+self.type+"_cls": self.softmax_cross_entropy_loss(),
            "loss_"+self.type+"_box_reg": self.smooth_l1_loss(),
        }
...
class FastRCNNOutputLayers(nn.Module):
    ...
    def __init__(self, input_size, num_classes, cls_agnostic_bbox_reg, box_dim=4,type="instance"):
        """
        Args:
...
class DenseFastRCNNOutputs(object):
    """
    A class that stores information about outputs of a Fast R-CNN head.
    """
    ...
    def binary_focal_loss(self, pos_gamma=1.0,neg_gamma=2.0):
        pred=self.pred_class_logits
        gt=self.gt_classes
        # print("======================================")
    ...
    def losses(self):
        ...
        return {
            "loss_"+self.type+"_cls": self.binary_focal_loss(),
            "loss_"+self.type+"_box_reg": self.smooth_l1_loss(),
        }
...
class DenseFastRCNNOutputLayers(nn.Module):
    """
    Two linear layers for predicting Fast R-CNN outputs:
      (1) proposal-to-detection box regression deltas
      (2) classification scores
    ...
    def __init__(self, input_size, num_classes, cls_agnostic_bbox_reg, box_dim=4,type="instance"):
        ...
        Args:
            ...
            box_dim (int): the dimension of bounding boxes.
                Example box dimensions: 4 for regular XYXY boxes and 5 for rotated XYWHA boxes
        """
        super(DenseFastRCNNOutputLayers, self).__init__()
        self.type=type
